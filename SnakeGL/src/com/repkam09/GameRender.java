...
public class GameRender {
	...
	public static void init() {
		try {
			...
			GL11.glLoadIdentity();
			GL11.glOrtho(0, GameEngine.GAME_BOARD_WIDTH, 0,
					GameEngine.GAME_BOARD_HEIGHT, 1, -1);
	...
	public static void checkUserInput() {
		...
		if (Keyboard.next() && Keyboard.getEventKeyState()) {
			...
			System.out
					.println("User Input: " + Keyboard.getKeyName(user_input));
			switch (user_input) {
			case Keyboard.KEY_UP:
				if (GameEngine.SNAKE_DIRECTION != GameEngine.SNAKE_DOWN) {
					GameEngine.SNAKE_DIRECTION = GameEngine.SNAKE_UP;
				}
				break;
			case Keyboard.KEY_DOWN:
				if (GameEngine.SNAKE_DIRECTION != GameEngine.SNAKE_UP) {
					GameEngine.SNAKE_DIRECTION = GameEngine.SNAKE_DOWN;
				}
				break;
			case Keyboard.KEY_LEFT:
				if (GameEngine.SNAKE_DIRECTION != GameEngine.SNAKE_RIGHT) {
					GameEngine.SNAKE_DIRECTION = GameEngine.SNAKE_LEFT;
				}
				break;
			case Keyboard.KEY_RIGHT:
				if (GameEngine.SNAKE_DIRECTION != GameEngine.SNAKE_LEFT) {
			...
			case Keyboard.KEY_ESCAPE:
				...
				break;
			default:
				System.out.println("Unhandled user input '"
						+ Keyboard.getKeyName(user_input) + "' [" + user_input
						+ "]");
				break;
			}
		}
	...
	public static void updateGameDisplay() {
		...
		for (int i = 0; i < GameEngine.GAME_BOARD_ARRAY.length; i++) {
			for (int j = 0; j < GameEngine.GAME_BOARD_ARRAY.length; j++) {
				...
				int x_adjust = j
						* (GameEngine.GAME_BOARD_WIDTH / GameEngine.GAME_BOARD_SIZE);
				int y_adjust = i
						* (GameEngine.GAME_BOARD_HEIGHT / GameEngine.GAME_BOARD_SIZE);
				...
				// System.out.println("tile [" + i + "," + j + "] render at {" +
				// x_adjust + "," + y_adjust + "} with size " + sizeX + "x" +
				// sizeY);
